...
def grid(rows,cols,size):
    """
    I have to iterate through the width and height of the canvas by deviding into the size of each grid. There is also the values to shift each grid to avoid overlapping for previous draw grids. 
    """
    for row in range(rows//size):
        for col in range(cols//size):
            # draw_square((rows// + (row * size) - 10),(cols//100 + (col * size + 1)-1),size)
            draw_square(col*size,row*size,size)
